...
import type { ValidationResultDto, ReleaseSnapshotDto } from '@the-crew/shared-types'
import {
...
export class ValidationService {
  ...
  async validate(projectId: string): Promise<ValidationResultDto> {
    const snapshot = await this.collectSnapshot(projectId)
    const issues = this.engine.validate(snapshot)
  ...
  private async collectSnapshot(projectId: string): Promise<ReleaseSnapshotDto> {
